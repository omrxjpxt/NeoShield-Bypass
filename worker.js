...
// Dedicated Mock Test Automation Backend Bridge
let isMockSolveInProgress = false;

chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'mockTestSolve') {
        if (isMockSolveInProgress) {
            console.warn('[Worker] Simultaneous solve request blocked.');
            sendResponse({
                success: false,
                error: 'A solve request is already in progress. Please wait.'
            });
            return true;
        }
        ...
        (async () => {
            ...
            try {
                ...
                if (request.data.type === 'mcq') {
                    if (typeof result.answerIndex !== 'number' || result.answerIndex < 0) {
                        throw new Error('Malformed AI response: "answerIndex" must be a non-negative integer.');
                    }
                } else {
                    if (!result.answer || typeof result.answer !== 'string') {
                        throw new Error('Malformed AI response: missing "answer" property.');
                    }
                }

                console.log('[Worker] Received validated structured answer:', result);
                sendResponse({
                    success: true,
                    data: result
                });
            } catch (err) {
                console.error('[Worker] Error communicating with mock backend:', err);
                sendResponse({
                    success: false,
        ...
        (async () => {
            try {
                ...
                if (resp.ok) {
                    ...
                    sendResponse({ online: false, error: `HTTP ${resp.status}` });
                }
            } catch (e) {
