document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateShortcutsForPlatform() {
        ...
        const shortcutMappings = {
            ...
            'Command + G': isMac ? 'Command + G' : 'Ctrl + G'
        };
...
async function fetchAccountInfo() {
    ...
    // Mock Test Sandbox Configuration
    const mockStatusInfo = document.getElementById('mockStatusInfo');
    const mockBackendUrlInput = document.getElementById('mockBackendUrl');
    const saveMockBackendUrlBtn = document.getElementById('saveMockBackendUrl');
    const openMockTestBtn = document.getElementById('openMockTestBtn');
    const mockShortcutKey = document.getElementById('mockShortcutKey');

    if (mockShortcutKey) {
        mockShortcutKey.textContent = isMac ? 'Command + G' : 'Ctrl + G';
    }
    ...
    async function checkMockBackendHealth() {
        ...
        if (mockStatusInfo) {
            mockStatusInfo.textContent = `Connecting to ${url}...`;
            mockStatusInfo.className = 'toggle-info';
        }

        chrome.runtime.sendMessage({
            action: 'getMockBackendStatus',
            backendUrl: url
        }, (response) => {
            if (chrome.runtime.lastError || !response || !response.online) {
                if (mockStatusInfo) {
                    mockStatusInfo.innerHTML = `🔴 Backend Offline (<span style="font-family:monospace">${url}</span>). Start backend in terminal.`;
